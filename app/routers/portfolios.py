...
def update_portfolio(item_id: int, body: PortfolioUpdate, conn=Depends(get_db), _=Depends(get_current_admin)):
    ...
    return execute_returning(conn,
        "UPDATE portfolios SET title=COALESCE(%s,title), category=COALESCE(%s,category), description=COALESCE(%s,description), image=COALESCE(%s,image), tags=COALESCE(%s::json,tags), client_name=COALESCE(%s,client_name), updated_at=now() WHERE id=%s RETURNING *",
