...
- Web service: `izonesite-backend`
- Database: `izonesite-db`
...
3. Connect the repository that contains the new `render.yaml`.
4. During Blueprint creation, provide values for:
   - `ADMIN_USERNAME`
   - `ADMIN_PASSWORD`
   - `ALLOWED_ORIGINS`
5. Confirm the resources and create the Blueprint.
6. After deploy completes, open:
   - `https://<your-backend>.onrender.com/api/health`
7. Verify it returns a healthy database response.

### Recommended Render Values

- `ALLOWED_ORIGINS`: set only your real frontend URLs
  - Example: `https://your-frontend.onrender.com,https://yourdomain.com`
- `DEBUG`: already pinned to `false` in the Blueprint
- `SECRET_KEY`: auto-generated by Render from the Blueprint
- `DATABASE_URL`: auto-linked from the Render Postgres instance
...
Change these plans in `render.yaml` before creating the Blueprint if you want paid production capacity.
